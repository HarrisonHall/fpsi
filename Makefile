...
dataframe.o: src/data/dataframe.cpp src/data/dataframe.hpp
	$(CXX) -Wall -c src/data/dataframe.cpp -o dataframe.o $(CXXFLAGS)

datahandler.o: src/data/datahandler.cpp src/data/datahandler.hpp
	$(CXX) -Wall -c src/data/datahandler.cpp -o datahandler.o $(CXXFLAGS)
...
gui.o: src/gui/gui.cpp src/gui/gui.hpp
	$(CXX) -Wall -c src/gui/gui.cpp -o gui.o `pkg-config gtkmm-3.0 --cflags --libs` $(CXXFLAGS)
...
plugin.o: src/plugin/plugin.hpp src/plugin/plugin_handler.cpp src/plugin/plugin_handler.hpp
	$(CXX) -Wall -c src/plugin/plugin_handler.cpp -o plugin.o $(CXXFLAGS)

sqlite.o:
	$(CC) -c include/sqlite3.c -o sqlite.o

util.o: src/util/logging.cpp src/util/logging.hpp src/util/time.cpp src/util/time.hpp
	$(CXX) -Wall -c src/util/logging.cpp -o logging.o $(CXXFLAGS)
	$(CXX) -Wall -c src/util/time.cpp -o time.o $(CXXFLAGS)
	ld -r logging.o time.o -o util.o

session.o: src/session/session.cpp src/session/session.hpp
	$(CXX) -Wall -c src/session/session.cpp -o session.o $(CXXFLAGS)

sessiongui.o: src/session/session.cpp src/session/session.hpp
	$(CXX) -Wall $(GUIFLAGS) -c src/session/session.cpp -o sessiongui.o `pkg-config gtkmm-3.0 --cflags --libs` $(CXXFLAGS)
...
main: dataframe.o datahandler.o gfx.o plugin.o sessiongui.o sqlite.o util.o
	$(CXX) $(LIBS) $(GUIFLAGS) dataframe.o datahandler.o gfx.o plugin.o sessiongui.o sqlite.o util.o src/main.cpp $(CXXFLAGS) `pkg-config gtkmm-3.0 --cflags --libs` -o fpsi -Ofast
